...
export async function getAllOffers(): Promise<Offer[]> {
  ...
  return snapshot.docs.map(docToOffer).sort(sortOffers);
}
...
export async function getActiveOffers(now = new Date()): Promise<Offer[]> {
  ...
  return snapshot.docs
    ...
    .filter((offer) => isOfferActiveRaw(offer, now))
    .sort(sortOffers);
}

export async function getCurrentOffer(now = new Date()): Promise<Offer | null> {
  const active = await getActiveOffers(now);
