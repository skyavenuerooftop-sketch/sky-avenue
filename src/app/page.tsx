...
import { menuItems } from "@/src/lib/menuData";
import { createMetadata } from "@/src/lib/seo";
import { getActiveOffers, getCurrentOffer } from "@/src/lib/offersFirebase";
...
export default async function HomePage() {
  ...
  // Get all active offers and map them for client components
  const activeOffers = await getActiveOffers();
  const heroOffers = activeOffers.map((o) => ({
    id: o.id,
    title: o.title,
  ...
  // Keep using the best "current" offer for the banner below hero
  const currentOffer = await getCurrentOffer();

  return (
    <>
      {/* Hero now receives offers for the rotating cards */}
      <Hero offers={heroOffers} />

      {/* Dynamic current offer banner below hero (optional, can remove if you feel it's duplicate) */}
      {/* {currentOffer && (
        <CurrentOfferBanner
          title={currentOffer.title}
          description={currentOffer.description}
      ...
      <section className="section section-padding">
        <div className="grid gap-10 lg:grid-cols-[1.2fr,1fr]">
          <div>
            <h2 className="heading-2">
              Contact our
              <span className="text-gold"> rooftop team.</span>
            </h2>
            <p className="mt-4 text-sm text-slate-300 sm:text-base">
              Ask about private events, large group bookings, or special
              occasions. Share your ideas and we&apos;ll help you plan the
              perfect evening above the city.
            </p>
            <ul className="mt-5 space-y-2 text-sm text-slate-300">
              <li>• Event planning support for birthdays & anniversaries.</li>
              <li>• Custom menus and beverage packages.</li>
              <li>• Full rooftop buy-outs for corporate events.</li>
            </ul>
          </div>
